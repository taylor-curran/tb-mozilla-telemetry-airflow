...
def _build_nested_struct(meta):
    """Build a nested StructType from a JSON schema node."""
    struct = StructType()
...
def _get_field_for_prop(prop, meta):
    """Map a single JSON schema property to a pyspark StructField."""
    nullable = "null" in meta["type"]
    meta_type = meta["type"]

    if "string" in meta_type:
        logging.debug(f"{prop!r} allows the type to be String AND Integer")
        return StructField(prop, StringType(), nullable)
    if "integer" in meta_type:
        return StructField(prop, IntegerType(), nullable)
    if "boolean" in meta_type:
        return StructField(prop, BooleanType(), nullable)
    if meta_type == "array" and "items" not in meta:
        return StructField(prop, ArrayType(StringType(), False), True)
    if meta_type == "array" and "items" in meta:
        return StructField(prop, ArrayType(_build_nested_struct(meta["items"])), True)
    if meta_type == "object":
        return StructField(prop, _build_nested_struct(meta), True)
...
def get_rows(schema):
    ...
    for prop in sorted(schema["properties"]):
        ...
        yield _get_field_for_prop(prop, meta)
...
def backfill(
    ...
    if source_gcs_path is None or dest_gcs_path is None:
        raise ValueError("backfill requires source_gcs_path and dest_gcs_path")
