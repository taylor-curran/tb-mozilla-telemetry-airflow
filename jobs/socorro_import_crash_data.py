...
def _build_nested_struct(schema):
    """Build a StructType from a nested JSON schema by recursively calling get_rows."""
    struct = StructType()
    for row in get_rows(schema):
        struct.add(row)
    return struct
...
def _map_property_to_field(prop, meta):
...
def get_rows(schema):
    ...
    for prop in sorted(schema["properties"]):
        ...
        yield _map_property_to_field(prop, meta)
