...
def _get_nested_struct(schema):
    """Build a StructType from a nested JSON schema."""
    struct = StructType()
    for row in get_rows(schema):
        struct.add(row)
    return struct
...
def _map_field(prop, meta):
    ...
    if meta["type"] == "array" and "items" in meta:
        return StructField(prop, ArrayType(_get_nested_struct(meta["items"])), True)
    if meta["type"] == "object":
        return StructField(prop, _get_nested_struct(meta), True)
