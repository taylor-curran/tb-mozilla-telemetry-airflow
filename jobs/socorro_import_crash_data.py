...
def _build_nested_struct(schema):
    """Build a StructType from a nested schema by recursively processing rows."""
    struct = StructType()
    for row in get_rows(schema):
        struct.add(row)
    return struct
...
def _resolve_field(prop, meta):
    """Resolve a single property in a JSON schema to a pyspark StructField."""
...
def get_rows(schema):
    ...
    for prop in sorted(schema["properties"]):
        ...
        yield _resolve_field(prop, meta)
