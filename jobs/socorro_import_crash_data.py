...
def _map_field(prop, meta):
    """Map a single JSON schema field to a pyspark StructField."""
...
def backfill(source_gcs_path, dest_gcs_path, start_date_yyyymmdd, schema, version, num_partitions):
    """
    Import data from a start date to yesterday's date.

    Example:
    -------
        backfill(source, dest, "20160902", crash_schema, version, 10)
    ...
    for d in daterange(start_date, end_date):
        try:
            import_day(source_gcs_path, dest_gcs_path, d, schema, version, num_partitions)
        except Exception as e:
            log.error(e)
