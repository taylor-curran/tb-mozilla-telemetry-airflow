...
DOCS = """
### Gmail Panopto integration
Runs a script in docker image that
 - will syncronize a Panopto user group with a Gmail user group
...
def get_airflow_log_link(context):
    ...
    base_url += "eam-gmail-panopto-integration/grid?tab=logs&dag_run_id="
...
def create_jira_ticket(context):
    ...
    summary = "Gmail Panopto Integration - Airflow Task Issue Exception"
...
gmail_panopto_gmail_client_x509_cert_url = create_secret(
...
with DAG(
    ...
    gmail_panopto_integration = GKEPodOperator(
        task_id="gmail_panopto_integration",
        arguments=[
            "python",
            "scripts/gmail_panopto_integration.py",
            "--level",
            "info",
        ],
